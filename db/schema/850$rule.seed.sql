...
BEGIN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM rule."fee" WHERE "conditionId" IN (SELECT "conditionId" FROM rule."condition" WHERE "priority" = 1 AND "channelCountryId" = 1)) THEN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM rule."limit" WHERE "conditionId" IN (SELECT "conditionId" FROM rule."condition" WHERE "priority" = 1 AND "channelCountryId" = 1)) THEN
